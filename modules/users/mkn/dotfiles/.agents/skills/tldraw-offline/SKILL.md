...
The default server is `http://localhost:7236`. If that port is not active, read the `port` from `/Users/mkn/Library/Application Support/tldraw/server.json`.
...
```bash
PORT=$(node -p 'JSON.parse(require("fs").readFileSync(process.argv[1],"utf8")).port' '/Users/mkn/Library/Application Support/tldraw/server.json'); TOKEN=$(node -p 'JSON.parse(require("fs").readFileSync(process.argv[1],"utf8")).token' '/Users/mkn/Library/Application Support/tldraw/server.json')
